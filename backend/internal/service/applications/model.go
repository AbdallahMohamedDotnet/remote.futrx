...
const (
	...
	// KindUI installs nothing in any container: the application is a browser-side
	// extension and its whole payload is its ui/ directory. No container, no
	// port, no proxy device, no install script.
	KindUI Kind = "ui"
	// KindBackend installs nothing in any container either: the application's
	// payload is the Go source under its backend/ directory, which the server
	// compiles and runs as a child process. It is what lets an application add a
