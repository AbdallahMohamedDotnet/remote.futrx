...
- **Version pinning:** agent CLI versions live in a single embedded manifest ([`agent-cli-versions.env`](backend/internal/agent/provisioning/agent-cli-versions.env)); host toolchain versions in [`infra/versions.env`](infra/versions.env). Several upstream fetches (NodeSource, the Go tarball, the code-server `.deb`, the Ubuntu base image) are version-pinned but not checksum- or signature-verified.
