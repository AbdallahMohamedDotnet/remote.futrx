...
finish. The `CheckTwoFactor`/`TwoFactorChallenge` step only exists for accounts
that have opted into TOTP 2FA (see below); every other account skips straight
from `SignIn` to `CheckLocalAdmin`, unchanged from before 2FA existed.
