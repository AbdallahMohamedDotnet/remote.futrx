...
- **No database.** Every authoritative datastore is a flat JSON file or per-chat JSONL log under `DATA_DIR` (`/opt/remote.futrx/data`). Chat offset/turn indexes live only in process memory and rebuild lazily from JSONL after restart. JSON and metadata writes use temp-file + rename, while chat events append directly to JSONL with `O_APPEND`. The authoritative paths use neither `fsync` nor file locking, so a crash, power loss, or stray second process can lose or corrupt state. *(`backend/internal/stores/`)*
...
- **Chat logs grow without bound; their process-local index cache is only approximately bounded.** Per-chat `events.jsonl` files have no rotation or compaction. The process retains compact event locations and transcript-turn ranges up to an approximate cache budget, evicting least-recently-used chats and rebuilding them on demand. A single chat larger than that budget remains cached, so practical scale is still bounded by both disk and host memory. Restart drops all indexes.
