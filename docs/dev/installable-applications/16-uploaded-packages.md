...
Everything else in this documentation describes applications that ship **inside the
server binary**: a directory under `applications/`, embedded at build time, released
...
browser, its `backend/` is compiled and run. Nothing downstream of the registry
...
$DATA_DIR/app-packages/
  applications/<id>/        the extracted package — exactly an applications/<id>/ directory
...
built-in application would not.
...
`"id"` has to be explicit here. A built-in application may omit it and inherit its
...
`"version"` is required of every application, built-in or uploaded, because it is
...
| Refused | Why |
|---|---|
| An id that a built-in application already uses | An upload may add an application, not redefine what `mysql` installs |
| An archive with no `application.json` | There is nothing to add |
| An `application.json` with no `version` | Without one, no later upload could ever upgrade it |
| A member path that escapes the application directory | The extractor never writes outside `applications/<id>/` |
...
| Anything `loadApplication` rejects | A package that would not have worked is refused at upload, not at install |
...
A copy whose application can no longer be loaded — a package broken by a server
...
disk; it is shadowed. The built-in application is what the catalog serves, what the
...
id, because those belong to the built-in application now and go on working once the
...
`backend/` is compiled and run as a child of the server process with the
...
into `applications/` — see [13 — Security model](13-security-model.md). Upload
...
| `409` | The id belongs to a built-in application |
...
| `409` | Still installed (the body names where), a copy could not be uninstalled, or the id is a built-in application |
