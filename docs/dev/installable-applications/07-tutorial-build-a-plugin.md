...
```bash
mkdir -p applications/open-in-cursor/ui/{scripts,style,views,assets}
```

The directory name is the application id. Use lowercase letters, digits, and dashes.

## 2. Write `application.json`

```bash
cat > applications/open-in-cursor/application.json <<'JSON'
{
  "id": "open-in-cursor",
  "name": "Open in Cursor",
...
```bash
cat > applications/open-in-cursor/ui/assets/logo.svg <<'SVG'
...
```bash
cat > applications/open-in-cursor/ui/scripts/main.js <<'JS'
...
```bash
cat > applications/open-in-cursor/ui/views/opened.html <<'HTML'
...
```bash
cat > applications/open-in-cursor/ui/style/cursor.css <<'CSS'
.cursor-ext__lead,
.cursor-ext__hint {
  margin: 0;
  font-size: 12px;
...
If `application.json` is malformed or the `ui` block names a missing file, this is
...
Nothing appears until the application is installed — that is deliberate, see
...
Because it is a `ui` application, install completes instantly and needs no LXD.
