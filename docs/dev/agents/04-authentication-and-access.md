...
| MiniMax | `managed-api-key` | An admin submits or removes a write-only key in Settings. Before saving, Remote verifies the key with the non-generation `GET /v1/models` endpoint; status exposes only whether a validated key exists. | Project-only. The host-managed key is injected as an environment variable into the isolated Codex process and is not written into its generated model catalog. |
