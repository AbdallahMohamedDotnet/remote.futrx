...
| MiniMax | `external` | No host login. Remote shows instructions for adding `MINIMAX_API_KEY` to a project. | Project-only. The key is passed as an environment variable to an isolated Codex home and is not written into its generated model catalog. |
