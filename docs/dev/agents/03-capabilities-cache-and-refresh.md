...
| MiniMax | Returns the provider-owned `MiniMax-M3` catalog with Think-Off/Adaptive reasoning and Default/Plan modes. It is project-scoped and performs no credential-bearing discovery probe. | The same static catalog remains available before `MINIMAX_API_KEY` is added; launch performs the authoritative project-secret check. |
