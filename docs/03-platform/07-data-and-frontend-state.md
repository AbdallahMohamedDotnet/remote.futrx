...
`DATA_DIR/transcript-index.sqlite` is derived from the chat JSONL logs. It is
backfilled lazily, refreshed incrementally, and cleaned up when a chat is
deleted. A rewind requests an immediate rebuild; the next indexed read can
recover from a missed refresh. Indexed-read failures fall back to scanning the
canonical JSONL file. The index can be deleted while the service is stopped and
will be rebuilt on subsequent chat access.
