...
### 8. Untrusted container poisons host-canonical tokens via sync-back — **High** ✓ code-verified — partially mitigated for saved Claude/Codex accounts

**Tampering.** After a successful run (`err == nil`), the host **pulls** credential files the container wrote back onto the canonical host paths, chmods 0600, and stamps `mtime` to now ([`provider.go`](../backend/internal/integration/agents/claude/provider.go), [`credentials/files.go`](../backend/internal/integration/containers/credentials/files.go)). The pull is **unconditional** (no mtime comparison), so a compromised agent that overwrites its own `/root/.claude/.credentials.json` and exits 0 gets its tokens onto the host. Because the host copy is then stamped newer (and Codex's `auth.json` is host-authoritative, pushed before every project run regardless of age), the next push re-pushes the poisoned tokens into **every other container**. The pull itself applies no signature, provenance, or content validation.

**Saved-account vault.** For Claude and Codex, once a saved account is active, the provider follows sync-back with the service-owned capture ([`account_service.go`](../backend/internal/service/agent/auth/account_service.go)), which admits a changed host login into `agent-accounts.json` only after provider validation and an identity match with the leased active account. A login that records another account, or none, is not saved, and the host is immediately rewritten with the saved active credential, so it is not re-pushed. Startup reconciliation and the start of every run likewise replace a host login that does not record the active account's identity. Codex's host-authoritative push overwrites the project's `auth.json` before every run, so each run starts from the host's record rather than one the container kept.

- **Existing mitigations:** sync-back only fires on a clean exit; directory sync uses basenames only (no path traversal); Codex re-checks auth mode after sync; with an active saved account, pulled credentials are validated and identity-checked before they enter the vault, and a foreign login is replaced on the host; accounts cannot be switched while a run holds the active one.
- **Residual gap:** "successful run" is fully attacker-controlled, and the pull still lands on the canonical host path before any check, so a run prepared in that window can receive the pulled file. A pulled credential that fails validation (possibly transiently) is neither saved nor restored: it stays on the host, where startup reconciliation also keeps it if it claims the active identity, until a later capture or an administrator's account activation replaces it, and it can be pushed to other containers meanwhile. Claude's identity comes from the local `oauthAccount` profile and its validation (`claude auth status` plus an expiry check) never contacts Anthropic, so a container that keeps or forges the active account's profile around foreign or unusable tokens is not detected: the tokens are saved into the vault and pushed fleet-wide. Codex validation refreshes the login through OpenAI, so its identity is checked against the server-issued ID token, not one the container wrote. Without an active saved account (for example, a host login from before saved accounts that was never imported), and for Kimi, the original behavior is unchanged: the host trusts whatever the container writes. A single container can therefore still cause a fleet-wide auth DoS for Claude, Codex, and Kimi, and fleet-wide credential poisoning for Claude and for providers without an active saved account. The vault itself is plaintext at rest (finding 18).
...
6. **Finish validating credential sync-back** (finding 8): pull into a staging location instead of the canonical host path, restore the saved login when validation fails, give Claude a server-backed identity check, and cover providers without saved accounts; scope provider tokens per project if feasible (findings 7, 8).
