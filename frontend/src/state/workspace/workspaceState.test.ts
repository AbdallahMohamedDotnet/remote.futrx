...
test("a chat created from this client opens instead of bouncing back", () => {
  ...
  assert.equal(workspaceSidebarState.isActiveChatMissing(listed, created.id), true);
  assert.equal(workspaceSidebarState.replacementChatId(listed), "loose");
  ...
  assert.equal(workspaceSidebarState.isActiveChatMissing(seeded, created.id), false);
  assert.equal(workspaceSidebarState.activeChat(seeded, created.id)?.id, created.id);
