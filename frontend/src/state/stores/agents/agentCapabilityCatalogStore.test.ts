...
test("keeps the previous response visible while consulting the shared backend cache", async () => {
  ...
  let calls = 0;
  const store = new AgentCapabilityCatalogStore(async () => {
    calls++;
    if (calls === 1) return catalog("initial");
    return new Promise((resolve) => { resolveRefresh = resolve; });
  });
  await store.load("user@example.com", "project-1");

  const refreshing = store.load("user@example.com", "project-1");
  const current = store.read("user@example.com", "project-1");
  ...
  await refreshing;
  assert.equal(
    store.read("user@example.com", "project-1").catalog?.providers[0]?.label,
...
test("coalesces simultaneous requests within one browser", async () => {
  ...
  const first = store.load("user@example.com", "project-1");
  const second = store.load("user@example.com", "project-1", { force: true });
...
test("manual refresh reaches the shared backend refresh path", async () => {
  ...
  const store = new AgentCapabilityCatalogStore(async (_projectId, options) => {
    refreshValues.push(options?.refresh);
    return catalog("loaded");
  });

  await store.load("user@example.com", "project-1");
  await store.load("user@example.com", "project-1", { force: true });
...
test("failed requests retain the last visible catalog", async () => {
  ...
  await store.load("user@example.com", "project-1");
  await assert.rejects(store.load("user@example.com", "project-1"));

  const snapshot = store.read("user@example.com", "project-1");
...
test("catalog rendering state remains isolated by user and project", async () => {
  const store = new AgentCapabilityCatalogStore(async (projectId) => catalog(projectId || "host"));
  await store.load("USER@example.com", "project-1");
  ...
  assert.equal(store.read("other@example.com", "project-1").catalog, null);
  assert.equal(store.read("user@example.com", "project-2").catalog, null);
});
