...
Every long-running piece of the stack is a systemd unit, so a crash gets
restarted automatically and a host reboot brings everything back without
intervention. There's **no separate frontend process** — the Preact SPA is
built into `backend/public/` at deploy time and embedded into the Go binary
via `//go:embed`, so serving the UI is the backend's job.
...
If a unit ever shows `inactive (dead)`, look at its `Restart=` directive
(`systemctl cat <unit>`) — that's the contract for whether it'll come back
on its own.
